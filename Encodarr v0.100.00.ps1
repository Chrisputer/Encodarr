...
$PVRAPI = "$89kjhwergt98q34trber98r309fch8"
...
$data | sort $_.directory.parent.name | sort $_.directory.name
...
foreach($mkv in $data)
{
   ...
   If($verif_input -eq $true)
   {
       ...
       Rename-Item -Path "$LibRoot\$show\$season\$episode$ext" -NewName "$episode.$env:computername"
       
       #--Handbrake the file with the proper name | You will need to specificy your preset to get this to work--#
       & "C:\Program Files\HandBrake\HandBrakeCLI.exe" --preset-import-file "$preset" -Z "custom" -i "$LibRoot$show\$season\$episode.$env:computername" -o "$LibRoot$show\$season\$episode.mkv"
       
       #--Removes the old (renamed) file--#
       Remove-Item "$LibRoot$show\$season\$episode.$env:computername"
